...
-> 环境变量 ``FOURESTDS_<SECTION>__<KEY>`` -> 代码传入的 overrides。
...
def _env_overrides() -> dict:
    """解析 FOURESTDS_SECTION__KEY=value 形式的环境变量。"""
    out: dict[str, dict[str, Any]] = {}
    for key, val in os.environ.items():
        if not key.startswith(_ENV_PREFIX) or key == "FOURESTDS_HOME":
            continue
