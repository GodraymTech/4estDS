...
def check_cog_format(image_path: str | Path) -> str:
    ...
    """
    if rasterio is None:
        log.warning("[cog] rasterio 未安装，无法执行 COG 检测。")
        return "invalid"

    path = Path(image_path)
    if not path.exists():
        log.error(f"[cog] 文件不存在: {path}")
        return "invalid"

    if path.suffix.lower() not in (".tif", ".tiff"):
        log.debug(f"[cog] 非 TIFF 后缀，跳过格式检测: {path.name}")
        return "invalid"
    ...
    try:
        ...
        log.error(f"[cog] 打开并检测 TIFF 格式失败 {path.name}: {e}")
        return "invalid"
...
def convert_to_cog(
    ...
    """
    if rasterio is None:
        log.error("[cog] rasterio 未安装，无法执行 COG 转换。")
        return False
    ...
    if not in_p.exists():
        log.error(f"[cog] 转换源文件不存在: {in_p}")
        return False

    log.info(f"[cog] 开始将 {in_p.name} 转换为 COG 格式...")
    try:
        ...
        with rasterio.open(out_p, "r+") as dst:
            ...
            if factors:
                log.debug(f"[cog] 构建金字塔 Overviews 层级因子: {factors}")
                resampling_map = {
        ...
        log.info(f"[cog] COG 转换完成: {out_p.name}")
        return True
    except Exception as e:
        log.exception(f"[cog] 转换 COG 失败: {e}")
        if out_p.exists():
            try:
                os.remove(out_p)
            except Exception:
