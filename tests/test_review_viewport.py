...
def test_replace_only_removes_unconfirmed_ai_in_scope() -> None:
    ...
    old_ai = {"id": "old", "source": "ai", "confirmed": False, "species": "B", "box_px": [50, 50, 60, 60]}
    outside_ai = {"id": "outside", "source": "ai", "confirmed": False, "species": "B", "box_px": [80, 80, 90, 90]}
    candidate = {"id": "new", "source": "ai", "confirmed": False, "species": "B", "box_px": [52, 52, 62, 62], "confidence": 0.9}

    result = ReviewMergeService().apply(
        "replace_ai_in_scope", [parent, human, old_ai, outside_ai], [candidate], (0, 0, 70, 70)
    )
    assert {item["id"] for item in result} == {"parent", "human", "outside", "new"}
