...
### Q1. 预推理的四叉树多尺度探针是用于全图正式推理吗？
**不是。** 四叉树（L0/L1/L2/L3）多尺度裁剪**仅在自标定采样阶段（SCOPE）**对极少量的采样种子窗口执行（全图面积占比仅 $\approx 1\%$），用以搜集不同分层分辨率下的单木表观尺寸。
而在对整张超大影像进行预处理（`preprocess`）或正式推理（`infer`）时，**一律使用根据自标定最优参数 $(T^*, r^*)$ 计算出的均匀重叠网格进行静态切片**。这避免了复杂的在线金字塔递归和拼缝错位，同时能够将 GPU 批量推理吞吐最大化。

### Q2. 为什么在代码中彻底用“重叠率”替代了“重叠像素”？
因为重叠像素（Overlap Pixels）会随着切片原生边长 $T$ 的变化而动态变化，在进行跨尺度参数寻优时由于基准不同，无法进行归一化对比。
因此，我们所有模块的输入、配置与求解均使用归一化的**重叠率 (overlap_rate: float, 范围 0.0 - 0.5)**；仅在最终构建读窗和像素裁剪时，在代码内部进行取整折算：`overlap_px = int(round(tile_size * overlap_rate))`。

### Q3. 在动态滑窗推理模式下，如何保障超大 TIFF 图像的读取效率？
对于普通的未分块（Striped）TIFF 图像，若执行频繁的局部滑动窗口随机读，会导致磁盘 I/O 磁头频繁寻道，性能极差。
为此，系统设计了**自适应影像路由与 COG 自动转换机制**：
- 如果输入是支持随机读取的 Tiled TIFF 或 Cloud Optimized GeoTIFF (COG)，系统会**跳过转码，直接路由至 `on_the_fly` 内存滑窗模式**，省去物理切片落盘时间和数万张瓦片带来的大量小文件写盘开销。
- 如果是非分块的普通 TIFF，在动态切片下系统会**在前置处理中强制自动执行 COG 转码**以构建瓦片网格与金字塔；如转换失败或输入不支持分块格式（如大 JPG/PNG），则**安全降级到静态物理切片落盘 (`physical_slice`)**。

下面是影像预处理路由与 COG 自动转换的详细逻辑判定流程图：

```mermaid
graph TD
    start([开始预处理: prepare_inference_image]) --> check_size{是否超出尺寸阈值?}
    
    check_size -->|否: 小图| direct[direct 模式: 直接送入模型推理]
    check_size -->|是: 超大图| check_format{是否为 TIFF 格式?}
    
    check_format -->|否: 超大JPG/PNG| physical[physical_slice 模式: 执行静态切片落盘]
    check_format -->|是: TIFF 影像| check_tiled{检查 Tiled/COG 状态}
    
    check_tiled -->|tiled_tiff 或 cog| skip_cog[跳过 COG 转换] --> check_action{用户指定 action?}
    check_tiled -->|normal_tiff: 未分块| convert_cog[强制自动执行 convert_to_cog 转换]
    
    convert_cog --> check_success{转换成功?}
    check_success -->|是| check_action
    check_success -->|否| force_physical[标记不支持 Tiled 随机读] --> physical
    
    check_action -->|默认/none| otf[on_the_fly 模式: 动态切片推理, 不落盘]
    check_action -->|slice: 强制限盘| physical
```

### Q4. 为什么否决了“连续非线性求解”，而采用离散参数网格寻优？
为了避免联合优化目标函数 $J(T, r)$ 的非凸性质导致求解陷入局部极小值；同时，为了保证项目的“轻依赖/无重依赖”原则，避免引入 `scipy.optimize` 这类沉重的外部库。
直接在符合深度学习感受野（32 像素倍数对齐）的离散参数格点 $\mathcal{T} \times \mathcal{R}$ 上做全空间网格遍历，不仅数值稳定性极佳，而且全局寻优的计算开销仅需 $0.1\,\text{ms}$，极其高效。

### Q5. 为什么合并去重算法（WBF / NMS）能从 $O(N^2)$ 降低到 $O(N)$？
在处理超大图（如 10万 $\times$ 10万 像素）的推理合并时，全局检出框往往高达 5 到 7 万个。如果采用暴力两两 IoU 对比去重，计算次数将达到几十亿次，会导致 CPU 主线程假死或死锁。
- **二维空间网格索引**：我们重构了 `wbf.py` 与 `scope.py` 的去重算法。利用检出框的最大尺寸动态划定二维网格（Cell），每个框在去重时只与其所在网格和相邻的 9 个网格内的其他框进行 IoU 运算，从而把全局对比转为了局部邻域对比。
- **DFS 栈结构优化**：在聚类连通分量的 DFS 中，用 Python 标准列表的 `pop()` 弹栈（$O(1)$ 复杂度）替换了原先的 `pop(0)` 队列操作（$O(M)$ 复杂度），彻底清除了内存移动开销。

### Q6. 大图推理合并时，为什么会因为 Loguru 日志输出导致解释器崩溃（段错误 SegFault）？
这是由于 Loguru 默认会拦截异常并生成非常详尽的 traceback 诊断日志（`_better_exceptions`）。
当推理过程发生异常时，Loguru 内部的序列化器会深度遍历当前栈帧中的本地变量。若当前帧存有大图上万个检测框的巨型 NumPy 数组，Loguru 在尝试解析其字符串 `repr` 表达时就会因耗尽 CPython 栈空间直接引发解释器段错误。
**解决方案**：在大图处理和批量调度等外层 `except` 捕获点，统一使用 `logger.opt(exception=False).error(...)` 代替 `logger.exception`，手动控制异常文本输出，从而避开堆栈溢出风险。

### Q7. 为什么 GIS 导出 Shapefile 失败却不影响推理？该如何补救？
为了保障核心推理流程的健壮性，系统在 `run_infer_pipeline` 末尾将 `export_tract_to_file` 进行了 `try-except` 包裹（GIS 导出失败不阻断程序），确保即使环境缺少地理库，已检出的数据也会安全地幂等登记进 SQLite 数据库中。
如果在推理运行时因为警告拦截或 Fiona/Geopandas 环境冲突发生报错（例如 `type object 'generator' has no attribute 'keys'`），您可以利用已经持久化在 DB 中的数据，通过独立的命令行直接执行补充导出，既安全又无需重新运行耗时的模型推理：
```bash
# 补充导出最新一次推理生成的 shapefile
uv run 4estds export --format shp
```
