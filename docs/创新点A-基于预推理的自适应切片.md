...
### 6.5 坐标回贴 + 跨尺度/窗口去重（空间网格加速） + 多尺度共识

- 用文件名解码把所有保留检测回贴到全图像素坐标。
- **跨尺度/窗口去重**：同一全图位置（IoU > 阈值）的检测判为同一棵树，合并。
- **二维空间网格索引去重加速**：为了应对超大图像探测样本堆叠和 WBF 融合时由于 $O(N^2)$ 两两比对引发的算力瓶颈与主线程挂起问题，合并算法被重构为基于 **二维空间网格索引 (Spatial Grid Indexing)** 机制。通过将坐标点依据框最大尺寸的 $1.5$ 倍网格化映射，将全局搜索优化为邻域 $9$ 宫格搜索，从而将去重开销大幅降低至 $O(N)$。同时，在连通图划分的 DFS 深度优先搜索中，采用栈的 $O(1)$ 弹栈替代了队列的 $O(M)$ 首端弹出。
...
### 6.8 全图静态均匀切片与自适应路由 → 正式检测

- **影像自适应路由与 COG 自动转换**：
  在预处理正式切片时，系统实现了智能化影像路由。对超大 JPG/PNG 等非空间栅格影像，一律降级到 `physical_slice`（静态切片落盘推理模式）；而对于 TIFF 格式，系统会自动检测其是否具备 `tiled_tiff` 或 `cog` 分块网格结构。对于已经分块的 TIFF，**跳过重复的转码转换，直接进行动态随机读**；而对于普通的未分块（striped）TIFF 影像，若在动态切片模式下，系统会**在前置处理中强制自动执行 COG 转码转换**，构建高效率瓦片和金字塔。这保证了后续内存滑动窗口随机读取数据时，拥有极速的 I/O 表现。
- **正式检测与 WBF 去重**：
  以求得的 `T*`、步距 `T*(1-r*)` 对影像做**静态均匀网格**切片推理（利用 nodata 积分图跳过无效空白块）。推理出的全图海量重叠检测框通过二维空间网格索引加速的尺度感知 WBF 进行去重融合，最后回贴地理空间坐标入库。
