...
- 所有子图**统一缩放到 640×640 并以 JPG 落盘**；缩放与反缩放一律基于像素，不引入米制单位。
- 命名结构清晰，按阶段进行划分：
  - **自标定采样探针（预推理阶段）**：
    保存于 `outputs/preprocess/scope_debug/{image_stem}_run_{run_id}/`，文件名格式为：
    `o{fx}_{fy}__s{T_size}_detected.jpg`
  - **正式切片落盘（预处理阶段）**：
    保存于 `outputs/preprocess/tiles__{image_stem}__run_{run_id}/`，文件名格式为：
    `o{wx}_{wy}__s{tile_size}.jpg`
    - `wx, wy` = 该子图左上角在全图中的像素原点坐标（整数）
    - `tile_size` = 该切片在全图分辨率下的原生边长像素（整数）

- **回贴公式**（检测框在推理子图内为 `bx,by,bw,bh`）：
  设原生边长为 $T$，缩放目标分辨率为 $P = 640$，则缩放比：
  $$\text{scale}=\frac{T}{P}$$
  回贴到全图坐标为：
  $$X = w_x + b_x \times \text{scale}$$
  $$Y = w_y + b_y \times \text{scale}$$
  $$W = b_w \times \text{scale}$$
  $$H = b_h \times \text{scale}$$

- 坐标全过程强制基于**整数像素**运算和命名，以防浮点数舍入带来的亚像素级缝隙偏差。
...
见第七节模型。**默认输出单一最优 (T*, r*)**；基于硬件对齐与运行稳定性，我们在离散的参数网格 $\mathcal{T} \times \mathcal{R}$ 上直接进行最优值求解，避免连续求解引入非凸优化陷入局部极值和复杂的第三方数学求解器依赖。
...
**(1) 边界截断概率**（窗口 T、重叠率 r、步距 s=T(1-r)，目标位置均匀）：
...
在离散网格候选空间中，直接搜索出使 $J$ 最大的最优组合 $(T^*, r^*)$。

**(4) 尺寸感知重叠约束**（保证大冠幅分位数 $d_{q95}$ 不被截断）：
$$r^{*} \ge \frac{d_{q95}}{T}$$
