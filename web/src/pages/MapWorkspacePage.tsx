...
export function MapWorkspacePage() {
    ...
    // 1. 监听底图鼠标 Hover 悬停检测框
    useEffect(() => {
        if (!map || !ready || !showDetections) {
            setHoveredDetection(null);
            return;
        }
        const nativeMap = map.getNativeMap() as any;
        if (!nativeMap || typeof nativeMap.on !== "function") return;

        const handleMouseMove = (e: any) => {
            const fillId = DETECTION_PREFIX + "fill";
            const lineId = DETECTION_PREFIX + "line";
            const availableLayers = [fillId, lineId].filter((id) => nativeMap.getLayer(id));
            if (!availableLayers.length) return;

            const features = nativeMap.queryRenderedFeatures(e.point, { layers: availableLayers });
        ...
        const handleClick = (e: any) => {
            ...
            if (features && features.length > 0) {
                ...
                const obsId = props.observation_id ?? props.id;
                if (obsId != null) {
                    navigator.clipboard.writeText(String(obsId)).then(() => {
                        message.success(`已复制观测 ID: #${obsId}`, 1.5);
                    }).catch(() => {
                        /* ignore fallback */
                    });
                }
        ...
        nativeMap.on("mousemove", handleMouseMove);
        nativeMap.on("mouseleave", handleMouseLeave);
        nativeMap.on("click", handleClick);
        return () => {
            nativeMap.off("mousemove", handleMouseMove);
            nativeMap.off("mouseleave", handleMouseLeave);
            nativeMap.off("click", handleClick);
        };
    }, [map, ready, showDetections]);

    // 监听 URL 中的 obs_id，定位并自动聚焦与高亮目标单木检测框
    const targetObsId = searchParams.get("obs_id");
    const focusedObsRef = useRef<string | null>(null);

    useEffect(() => {
        if (!map || !ready || !targetObsId || !visibleObservations) return;
        if (focusedObsRef.current === targetObsId) return;

        const targetFeature = visibleObservations.features.find((f: any) => {
            const p = f.properties || {};
            const id = p.observation_id ?? p.id;
            return String(id) === String(targetObsId);
        });

        if (!targetFeature) return;

        let centerLngLat: [number, number] | null = null;
        let obsBbox: [[number, number], [number, number]] | null = null;

        if (targetFeature.geometry) {
            const geom = targetFeature.geometry as any;
            ...
            if (geom.type === "Point" && Array.isArray(geom.coordinates)) {
                const px = Number(geom.coordinates[0]);
                const py = Number(geom.coordinates[1]);
                ...
                if (minX < Infinity && minY < Infinity) {
                    centerLngLat = [(minX + maxX) / 2, (minY + maxY) / 2];
                    if (maxX > minX && maxY > minY) {
                        obsBbox = [[minX, minY], [maxX, maxY]];
                    }
                }
            }
        }

        if (!centerLngLat || isNaN(centerLngLat[0]) || isNaN(centerLngLat[1])) return;

        focusedObsRef.current = targetObsId;

        // 1. 优先使用单木真实几何包围盒进行 fitBounds 缩放到“小头顶”视距；若无多边形则使用高放大率 flyTo
        if (obsBbox) {
            map.fitBounds(obsBbox, { padding: 160, maxZoom: 20.5, duration: 600 });
        } else {
            map.flyTo(centerLngLat, 20);
        }

        // 2. 自动高亮并展示 Tooltip
        const nativeMap = map.getNativeMap() as any;
        const triggerHighlight = () => {
            if (!nativeMap || typeof nativeMap.project !== "function") return;
            const screenPt = nativeMap.project(centerLngLat);
            setHoveredDetection({
                feature: targetFeature as any,
                x: screenPt.x,
                y: screenPt.y,
            });
        };
        ...
        triggerHighlight();
        const timer1 = setTimeout(triggerHighlight, 250);
        const timer2 = setTimeout(triggerHighlight, 700);
        return () => {
            clearTimeout(timer1);
            clearTimeout(timer2);
        };
    }, [map, ready, targetObsId, visibleObservations]);

    // 2. 悬停检测框时强化框内浅色填充高亮与边框亮化
    useEffect(() => {
        if (!map || !ready) return;
        const hoverFillId = DETECTION_PREFIX + "hover_fill";
        const hoverLineId = DETECTION_PREFIX + "hover_line";
        map.removeLayer(hoverFillId);
        map.removeLayer(hoverLineId);

        if (!hoveredDetection || !hoveredDetection.feature) return;

        const feature = hoveredDetection.feature;
        const rawSpecies = feature.properties?.species;
        const species = typeof rawSpecies === "string" && rawSpecies.trim() ? rawSpecies : "未知树种";
        const featColor = speciesColors.get(species) ?? speciesColor(species, 0);
        ...
        map.setGeoJsonLayer({
            ...
            color: featColor,
            opacity: 0.38,
            data: { type: "FeatureCollection", features: [feature] } as GeoJson,
        });
    ...
    }, [hoveredDetection, map, ready, speciesColors]);
    ...
    useEffect(() => {
        ...
        // 若存在 obs_id 指定观察目标，跳过全局 TIFF fitBounds，由单木小视角全权掌控
        if (targetObsId) return;
    ...
    }, [activeTract, effectiveArea.data?.boundary_geometry, groupByTract, map, ready, requestedTiff, targetObsId, visibleObservations]);
    ...
    return (
        <div style={ROOT}>
            ...
            {showDetections && hoveredDetection ? (
                <DetectionTooltip hover={hoveredDetection} colors={speciesColors} />
            ) : null}
...
function buildSpeciesLayers(
    ...
    const colorExpr = speciesColorExpression(species, colors);
    return [
        {
            id: DETECTION_PREFIX + "fill",
...
function getPxSize(props: Record<string, any>): string {
    let w: number | null = props.crown_width_px != null ? Number(props.crown_width_px) : null;
    let h: number | null = props.crown_height_px != null ? Number(props.crown_height_px) : null;

    if ((w == null || isNaN(w) || h == null || isNaN(h)) && props.box_px) {
        let box = props.box_px;
        ...
        if (Array.isArray(box) && box.length >= 4) {
            const b0 = Number(box[0]);
            const b1 = Number(box[1]);
            const b2 = Number(box[2]);
            const b3 = Number(box[3]);
    ...
    if (w != null && !isNaN(w) && h != null && !isNaN(h) && w > 0 && h > 0) {
        return `${Math.round(w)} x ${Math.round(h)}`;
    }
    return "-";
}

function DetectionTooltip({
    hover,
    colors,
}: {
    hover: { feature: GeoFeature; x: number; y: number };
    colors: Map<string, string>;
}) {
    const props = (hover.feature.properties || {}) as Record<string, any>;
    const rawSpecies = props.species;
    const species = typeof rawSpecies === "string" && rawSpecies.trim() ? rawSpecies : "未知树种";
    const tagColor = colors.get(species) ?? speciesColor(species, 0);

    const confidence = props.confidence != null ? Number(props.confidence).toFixed(2) : null;
    const heightVal = props.height != null ? Number(props.height).toFixed(2) : null;

    const sizePx = getPxSize(props);

    const widthGeo = props.crown_width_geo != null ? Number(props.crown_width_geo).toFixed(2) : null;
    const heightGeo = props.crown_height_geo != null ? Number(props.crown_height_geo).toFixed(2) : null;
    const sizeM = widthGeo && heightGeo ? `${widthGeo} x ${heightGeo}` : "-";

    const rawAreaGeo = props.crown_area_geo_real ?? props.crown_area_geo_est;
    const areaM2 = rawAreaGeo != null ? Number(rawAreaGeo).toFixed(2) : "-";

    const obsId = props.observation_id ?? props.id ?? null;

    const left = Math.min(hover.x + 14, window.innerWidth - 180);
    const top = Math.max(16, hover.y - 90);
    ...
    return (
        ...
        >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4, gap: 8 }}>
                <span style={{ fontWeight: 600, color: tagColor, fontSize: 12, lineHeight: 1.2, whiteSpace: "nowrap" }}>{species}</span>
                {confidence ? (
                    <span style={{ fontSize: 10, background: "var(--glass-chip)", color: "var(--glass-muted)", padding: "1px 5px", borderRadius: 4, lineHeight: 1.2, whiteSpace: "nowrap" }}>
                        {confidence}
                    </span>
                ) : null}
            </div>

            <div style={{ display: "grid", gap: 2, fontSize: 11, color: "var(--glass-muted)", lineHeight: 1.35 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ color: "var(--glass-text)", fontWeight: 500, whiteSpace: "nowrap" }}>尺寸(px):</span>
                    <span style={{ whiteSpace: "nowrap" }}>{sizePx}</span>
                </div>

                <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ color: "var(--glass-text)", fontWeight: 500, whiteSpace: "nowrap" }}>尺寸(m):</span>
                    <span style={{ whiteSpace: "nowrap" }}>{sizeM}</span>
                </div>

                <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <span style={{ color: "var(--glass-text)", fontWeight: 500, whiteSpace: "nowrap" }}>面积(m²):</span>
                    <span style={{ whiteSpace: "nowrap" }}>{areaM2}</span>
                </div>

                {heightVal ? (
                    <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                        <span style={{ color: "var(--glass-text)", fontWeight: 500, whiteSpace: "nowrap" }}>树高(m):</span>
                        <span style={{ whiteSpace: "nowrap" }}>{heightVal}</span>
                    </div>
                ) : null}

                {obsId ? (
                    <div style={{ fontSize: 10, color: "var(--glass-muted)", opacity: 0.7, textAlign: "right", marginTop: 1, whiteSpace: "nowrap" }}>
                        #{obsId}
                    </div>
                ) : null}
            </div>
