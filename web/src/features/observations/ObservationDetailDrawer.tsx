...
export function ObservationDetailDrawer({ open, onClose, item }: ObservationDetailDrawerProps) {
    ...
    const handleOpenMap = () => {
        const queryStr = item.observation_id ? `?obs_id=${encodeURIComponent(item.observation_id)}` : "";
        if (item.city && item.county && item.tract_id && item.phase_id) {
            ...
            navigate(`/map?tract_id=${encodeURIComponent(item.tract_id)}` + (item.observation_id ? `&obs_id=${encodeURIComponent(item.observation_id)}` : ""));
        } else {
            navigate("/map" + queryStr);
        }
    };

    const formatGeom = (val: unknown): string | null => {
    ...
    const boxPxStr = formatGeom(item.box_px);
    const boxPxSubStr = formatGeom(item.box_px_sub);
    const centerGeomStr = formatGeom(item.center_geom);
    const crownGeomStr = formatGeom(item.crown_geom);
        ...
        >
            ...
            <div style={{ marginBottom: 16 }}>
                ...
                <Descriptions column={2} size="small" bordered>
                    ...
                    <Descriptions.Item label="树高">
                        {item.height != null ? item.height.toFixed(2) : "-"}
                    </Descriptions.Item>
                    <Descriptions.Item label="树高来源">
                        {item.height_source || "未标明"}
                    </Descriptions.Item>
                    <Descriptions.Item label="冠幅长(m)">
                        {item.crown_width_geo != null ? item.crown_width_geo.toFixed(2) : "-"}
                    </Descriptions.Item>
                    <Descriptions.Item label="冠幅宽(m)">
                        {item.crown_height_geo != null ? item.crown_height_geo.toFixed(2) : "-"}
                    </Descriptions.Item>
                    <Descriptions.Item label="估算冠幅面积">
                        {item.crown_area_geo_est != null ? item.crown_area_geo_est.toFixed(2) : "-"}
                    </Descriptions.Item>
                    <Descriptions.Item label="真实冠幅面积">
                        {item.crown_area_geo_real != null ? item.crown_area_geo_real.toFixed(2) : "-"}
                    </Descriptions.Item>
                    <Descriptions.Item label="估算树冠体积">
                        {item.crown_volume_geo_est != null ? item.crown_volume_geo_est.toFixed(2) : "-"}
                    </Descriptions.Item>
                    <Descriptions.Item label="真实树冠体积">
                        {item.crown_volume_geo_real != null ? item.crown_volume_geo_real.toFixed(2) : "-"}
                    </Descriptions.Item>
                </Descriptions>
            </div>
            ...
            <div>
                ...
                <div style={{ display: "grid", gap: 12 }}>
                    {boxPxStr && (
                        <div>
                            <Text strong style={{ fontSize: 12 }}>全图像素检测框:</Text>
                            <Paragraph
                                code
                                copyable
                                style={{
                    ...
                    {boxPxSubStr && (
                        <div>
                            <Text strong style={{ fontSize: 12 }}>切片像素检测框:</Text>
                            <Paragraph
                                code
                                copyable
                                style={{
                    ...
                    {centerGeomStr && (
                        <div>
                            <Text strong style={{ fontSize: 12 }}>树冠中心点几何:</Text>
                            <Paragraph
                                code
                                copyable
                                style={{
                    ...
                    {crownGeomStr && (
                        <div>
                            <Text strong style={{ fontSize: 12 }}>树冠多边形轮廓:</Text>
                            <Paragraph
                                code
                                copyable
